<?php
/**
 * Main Class file for `MTP_Dashboard_Settings`
 *
    ...
    class MTP_Dashboard_Settings {
        ...
        public static function get_instance(){
            if (!isset(self::$instance)){
                self::$instance = new MTP_Dashboard_Settings();
            }
            ...
            foreach ( $this->sections_array as $section ) {
                ...
                if ( isset( $section['desc'] ) && ! empty( $section['desc'] ) ) {
                    ...
                    $callback = function() use ( $section ) {
                        print str_replace( '"', '\"', $section['desc'] );
                    };
        ...
        function callback_html( $args ) {
            print $this->get_field_description( $args );
        }
        /**
        ...
        function callback_wysiwyg( $args ) {
            ...
            print '<div style="max-width: ' . esc_attr__($size, 'food-to-prep') . ';">';
            ...
            wp_editor( $value, esc_attr__($args['section'], 'food-to-prep') . '-' . esc_attr__($args['id'], 'food-to-prep'), $editor_settings );
            print '</div>';
            print $this->get_field_description( $args );
        }
        /**
        ...
        function callback_file( $args ) {
            ...
            $label = isset( $args['options']['button_label'] ) ?
                ...
                __( 'Choose File', 'food-to-prep' );
            $html  = sprintf( '<input type="text" class="%1$s-text wpsa-url" id="%2$s[%3$s]" name="%2$s[%3$s]" value="%4$s"/>', esc_attr__($size, 'food-to-prep') , esc_attr__($args['section'], 'food-to-prep') , esc_attr__($args['id'], 'food-to-prep') , esc_attr__($value, 'food-to-prep') );
            $html .= '<input type="button" class="button wpsa-browse" value="' . esc_attr__($label, 'food-to-prep') . '" />';
            $html .= $this->get_field_description( $args );
            print $html;
        }
        /**
...
//            $type = isset( $args['type'] ) ? $args['type'] : 'separator';
        ...
        function get_option( $option, $section, $default = '' ) {
            ...
            if (!isset($this->sections_data[$section])){
                $this->sections_data[$section] = get_option( $section );
            }

            $options = $this->sections_data[$section];
        ...
        function show_navigation() {
            ...
            foreach ( $this->sections_array as $tab ) {
                $html .= sprintf( '<a href="#%1$s" class="nav-tab" id="%1$s-tab">%2$s</a>', esc_attr__( $tab['id'], 'food-to-prep' ), esc_html__($tab['title'], 'food-to-prep') );
            }
            $html .= '</h2>';
            print $html;
        }
        ...
        function show_forms() {
            ...
            <div class="metabox-holder">
                <?php foreach ( $this->sections_array as $form ) { ?>
                    <!-- style="display: none;" -->
                    <div id="<?php esc_attr_e($form['id'], 'food-to-prep'); ?>" class="group" >
    ...
    } // MTP_Dashboard_Settings ended.

    function MTP_OSA(){
        return MTP_Dashboard_Settings::get_instance();
    }
endif;